...
public class GlobalExceptionHandler {
    ...
    public ResponseEntity<ApiCallError<String>> handleBadCredentialsException(HttpServletRequest request, LockedException ex) {
        logger.error("LockedException {}\n", request.getRequestURI(), ex);

        return ResponseEntity
                .status(HttpStatus.BAD_REQUEST)
                .body(new ApiCallError<>("Account is locked", List.of(ex.getMessage())));
    }
    ...
    public ResponseEntity<ApiCallError<String>> handleBadRequestException(HttpServletRequest request, BadRequestException ex) {
        ...
        List<String> messages = ex.getMessage() == null ? List.of() : List.of(ex.getMessage());
        return ResponseEntity
                .status(HttpStatus.BAD_REQUEST)
                .body(new ApiCallError<>("Bad request", messages));
    }
    ...
    public ResponseEntity<ApiCallError<String>> handleDataIntegrityViolationException(HttpServletRequest request, DataIntegrityViolationException ex) {
        ...
        String cause = ((ConstraintViolationException) ex.getCause()).getConstraintName().split("\\.")[1];


        return ResponseEntity
                .status(HttpStatus.BAD_REQUEST)
                .body(new ApiCallError<>(DataIntegrityExceptions.MESSAGES.get(cause), List.of(ex.getMessage())));
    }
